...
import { Table, Typography, message, Tooltip, Modal, Input, Button } from "antd";
...
export default function UserComplaints() {
    ...
    useEffect(() => {
        ...
        const fetchUserComplaint = async () => {
            try {
                const response = await axios.get("http://localhost:5000/api/complaint/fetchUser-complaints");
                    ...
                    <>
                        <Tooltip title="Click to respond to user">
                            <ChatBubbleOutlineIcon
                                ...
                                style={{ color: "#164863", fontSize: 20, marginRight: 25, cursor: 'pointer' }}
                            />
                        </Tooltip>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', overflowX: 'hidden' }}>
            <NavbarAdmin />
            <Typography.Title level={2} style={{ marginTop: '2rem', marginLeft: '12px' }}>User Complaints</Typography.Title>
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key="submit" style={{ backgroundColor: '#3bb19b', color: 'white' }} onClick={handleOk}>
                        Send
                    </Button>,
                ]}
            >
                <Input.TextArea rows={4} value={adminResponse} onChange={(e) => setAdminResponse(e.target.value)} placeholder="Type your response here" />
            </Modal>
